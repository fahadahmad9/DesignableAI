...
const MATERIAL_DATA={
  leather:{"Smooth":[{src:"/materials/leather/Smooth/brown_1.jpg",name:"Brown"},{src:"/materials/leather/Smooth/yellow_1.jpg",name:"Yellow"},{src:"/materials/leather/Smooth/Blue.jpg",name:"Blue"},{src:"/materials/leather/Smooth/Grey.jpg",name:"Grey"},{src:"/materials/leather/Smooth/White.jpg",name:"White"}],"Stitched":[{src:"/materials/leather/Stitched/brown_stiched.jpg",name:"Brown"},{src:"/materials/leather/Stitched/grey_stitched.jpg",name:"Grey"},{src:"/materials/leather/Stitched/white_sticthed.jpg",name:"White"}],"Pebbled":[{src:"/materials/leather/Pebbled/Red.jpg",name:"Red"},{src:"/materials/leather/Pebbled/white.jpg",name:"White"}],"Suede":[{src:"/materials/leather/Suede/suede_brown.png",name:"Brown"}],"Rough":[{src:"/materials/leather/Rough/rough_grey.png",name:"Grey"}]},
  fabric:{"Cotton":[{src:"/materials/fabric/Cotton/blue.jpg",name:"Blue"},{src:"/materials/fabric/Cotton/grey.jpg",name:"Grey"},{src:"/materials/fabric/Cotton/olive.jpg",name:"Olive"},{src:"/materials/fabric/Cotton/silver.jpg",name:"Silver"}],"Pattern":[{src:"/materials/fabric/Pattern/red.jpg",name:"Red"},{src:"/materials/fabric/Pattern/white.jpg",name:"White"}],"Tiled":[{src:"/materials/fabric/Tiled/blue.jpg",name:"Blue"},{src:"/materials/fabric/Tiled/red.jpg",name:"Red"},{src:"/materials/fabric/Tiled/grey.jpg",name:"Grey"}],"Printed":[{src:"/materials/fabric/Printed/sky.jpg",name:"Sky"},{src:"/materials/fabric/Printed/white.jpg",name:"White"}],"Woven":[{src:"/materials/fabric/Woven/charcoal.jpg",name:"Charcoal"},{src:"/materials/fabric/Woven/grey.jpg",name:"Grey"},{src:"/materials/fabric/Woven/red.jpg",name:"Red"},{src:"/materials/fabric/Woven/silver.jpg",name:"Silver"}],"Stitched":[{src:"/materials/fabric/Sticthed/blue.jpg",name:"Blue"},{src:"/materials/fabric/Sticthed/black.jpg",name:"Black"},{src:"/materials/fabric/Sticthed/red.jpg",name:"Red"}],"Rough":[{src:"/materials/fabric/Rough/grey.jpg",name:"Grey"},{src:"/materials/fabric/Rough/silver.jpg",name:"Silver"},{src:"/materials/fabric/Rough/light_silver.jpg",name:"Light Silver"}]},
  metal:{"Brushed":[{src:"/materials/metal/Brushed/grey.jpg",name:"Grey"},{src:"/materials/metal/Brushed/silver.jpg",name:"Silver"},{src:"/materials/metal/Brushed/grey_2.jpg",name:"Grey II"},{src:"/materials/metal/Brushed/silver_2.jpg",name:"Silver II"}],"Matte":[{src:"/materials/metal/Matte/black.jpg",name:"Black"},{src:"/materials/metal/Matte/matte.jpg",name:"Matte"}],"Rusty":[{src:"/materials/metal/Rusty/rusty.jpg",name:"Rust I"},{src:"/materials/metal/Rusty/rusty_2.jpg",name:"Rust II"},{src:"/materials/metal/Rusty/rusty_3.jpg",name:"Rust III"}],"Chrome":[{src:"/materials/metal/Chrome/white.jpg",name:"Chrome"}]},
  wood:{"Walnut":[{src:"/materials/wood/Walnut/walnut_1.jpg",name:"Walnut I"},{src:"/materials/wood/Walnut/walnut_2.jpg",name:"Walnut II"},{src:"/materials/wood/Walnut/walnut_3.jpg",name:"Walnut III"},{src:"/materials/wood/Walnut/walnut_4.jpg",name:"Walnut IV"},{src:"/materials/wood/Walnut/walnut_5.jpg",name:"Walnut V"},{src:"/materials/wood/Walnut/walnut_6.jpg",name:"Walnut VI"}],"Maple":[{src:"/materials/wood/Maple/maple_1.jpg",name:"Maple I"},{src:"/materials/wood/Maple/maple_2.jpg",name:"Maple II"}],"Oak":[{src:"/materials/wood/Oak/oak_1.jpg",name:"Oak I"},{src:"/materials/wood/Oak/oak_2.jpg",name:"Oak II"}],"Cherry":[{src:"/materials/wood/Cherry/cherry.jpg",name:"Cherry"}]},
};
...
function findInventoryItemByType(partType){
  for(const category of INVENTORY){
    const item=category.items.find(entry=>entry.type===partType);
    if(item)return item;
  }
  return INVENTORY[0]?.items[0]||null;
}
...
export default function SculptStudio(){
  ...
  const[textures,setTextures]=useState({}); // partId -> src string
  const texLoaderRef=useRef(new THREE.TextureLoader());
  const texCacheRef=useRef({}); // src -> THREE.Texture
...
/* ── Material Engine ────────────────────────────────────────────── */
  const applyMaterial=useCallback((src)=>{
    if(!selectedPartId)return;
    const part=partsRef.current.find(p=>p.id===selectedPartId);
    if(!part)return;
    ...
    const applyTex=(tex)=>{
      ...
      tex.repeat.set(2,2); // tile the texture so it doesn't stretch
      // Replace mesh material with textured version, keep roughness/metalness
      ...
      setTextures(prev=>({...prev,[selectedPartId]:src}));
    };

    if(texCacheRef.current[src]){
      applyTex(texCacheRef.current[src]);
    }else{
      texLoaderRef.current.load(src,(tex)=>{
  ...
  const clearMaterial=useCallback(()=>{
    ...
    const part=partsRef.current.find(p=>p.id===selectedPartId);
    if(!part)return;
    // Dispose textured material and restore clay
    if(part.mesh.material.map)part.mesh.material.map=null;
    part.mesh.material.dispose();
    part.mesh.material=clayMat(selectedPartId===selectedPartId?CLAY_SEL:CLAY);
    part.mesh.material.color.set(CLAY_SEL); // keep selected highlight
    setTextures(prev=>{const n={...prev};delete n[selectedPartId];return n;});
  },[selectedPartId]);
  ...
  const applySculptBrush=useCallback((hit,mesh,invert,deltaWorld)=>{
    ...
    // SIDE SCALE — runs outside per-vertex loop, affects entire side
    if(brushType==="sidescale"){
      // Find which axis the clicked face normal is closest to
      const fn=normal; // face normal in local space
      const ax=Math.abs(fn.x),ay=Math.abs(fn.y),az=Math.abs(fn.z);
      // Dominant axis: whichever component is largest
      let axis,sign;
      if(ax>=ay&&ax>=az){axis="x";sign=fn.x>0?1:-1;}
      else if(ay>=ax&&ay>=az){axis="y";sign=fn.y>0?1:-1;}
      else{axis="z";sign=fn.z>0?1:-1;}
      // Flip to opposite side if Shift held
      if(invert)sign*=-1;
      // Find mesh center on this axis
      let minV=Infinity,maxV=-Infinity;
      for(let j=0;j<pos.count;j++){
        const v=axis==="x"?pos.getX(j):axis==="y"?pos.getY(j):pos.getZ(j);
        if(v<minV)minV=v;if(v>maxV)maxV=v;
      }
      const center=(minV+maxV)/2;
      const delta=str*8; // how much to move per frame
      // Move all vertices on the chosen side (those past center in sign direction)
      for(let i=0;i<pos.count;i++){
        const v=axis==="x"?pos.getX(i):axis==="y"?pos.getY(i):pos.getZ(i);
        if(v*sign>center*sign){ // vertex is on the selected side
          if(axis==="x")pos.setX(i,pos.getX(i)+sign*delta);
          else if(axis==="y")pos.setY(i,pos.getY(i)+sign*delta);
          else pos.setZ(i,pos.getZ(i)+sign*delta);
        }
      }
      pos.needsUpdate=true;geo.computeVertexNormals();
      return; // skip per-vertex loop below
    }
  ...
  const clearStudio=useCallback((options={})=>{
    if(sceneRef.current){
      partsRef.current.forEach(part=>sceneRef.current.remove(part.mesh));
      snapGlowsRef.current.forEach(glow=>sceneRef.current.remove(glow));
    }

    partsRef.current=[];
    snapGlowsRef.current=[];
    setPlacedParts([]);
    setSelectedPartId(null);
    setHasSeat(false);
    partIdCounter.current=0;
    neighborMapsRef.current={};
    ...
    movingRef.current={active:false,plane:null,offset:new THREE.Vector3()};
    sculptingRef.current=false;
    setTextures({});
    setOpenCat(null);
    setToolMode("select");

    if(options.closeDialogs!==false){
      setShowSaveModal(false);
      setShowProjectsModal(false);
    }
  },[]);

  const applyTextureToMesh=useCallback((mesh,src)=>{
    if(!src||!mesh)return;
    ...
    const applyTex=tex=>{
      ...
      mesh.material=new THREE.MeshStandardMaterial({map:tex,roughness:0.8,metalness:0.05});
    };
  ...
  const loadProjectIntoStudio=useCallback((project)=>{
    ...
    const savedTextures=data.textures||{};

    clearStudio({closeDialogs:false});

    const nextPlacedParts=[];
    const nextTextures={};
    const nextNeighborMaps={};
    const nextUndoStack=[];
    let maxPartIndex=-1;

    savedParts.forEach((savedPart,index)=>{
      const inventoryItem=findInventoryItemById(savedPart.inventoryId)||findInventoryItemByType(savedPart.type);
      if(!inventoryItem)return;

      const geo=inventoryItem.build();
      const positionAttribute=geo.attributes.position;
      const savedPositions=Array.isArray(savedPart.positions)?savedPart.positions:null;
      if(savedPositions&&savedPositions.length===positionAttribute.count*3){
      ...
      const mesh=new THREE.Mesh(geo,clayMat());
      const partId=savedPart.id||`part_${index}`;
      const position=savedPart.position||{};
      const scale=savedPart.scale||{};
      mesh.position.set(position.x||0,position.y||0,position.z||0);
      mesh.scale.set(scale.x||1,scale.y||1,scale.z||1);
      mesh.castShadow=true;
      mesh.receiveShadow=true;
      mesh.userData={id:partId,type:savedPart.type,inventoryId:inventoryItem.id};
      sceneRef.current?.add(mesh);

      if(savedPart.textureSrc||savedTextures[partId]){
        const textureSrc=savedPart.textureSrc||savedTextures[partId];
        nextTextures[partId]=textureSrc;
        applyTextureToMesh(mesh,textureSrc);
      }

      nextNeighborMaps[partId]=getNeighborMap(geo);
      nextUndoStack.push({id:partId,positions:clonePos(geo)});
      partsRef.current.push({id:partId,type:savedPart.type,inventoryId:inventoryItem.id,mesh});
      nextPlacedParts.push({id:partId,type:savedPart.type,name:inventoryItem.name});

      const numericId=parseInt(String(partId).replace(/[^0-9]/g,""),10);
      if(!Number.isNaN(numericId))maxPartIndex=Math.max(maxPartIndex,numericId);
    });

    neighborMapsRef.current=nextNeighborMaps;
    undoStackRef.current=nextUndoStack;
    setPlacedParts(nextPlacedParts);
    setTextures(nextTextures);
    setHasSeat(nextPlacedParts.some(part=>part.type==="seat"));
    setSelectedPartId(nextPlacedParts[0]?.id||null);
    setToolMode("select");
    setShowSaveModal(false);
    setShowProjectsModal(false);

    if(maxPartIndex>=0){
      partIdCounter.current=maxPartIndex+1;
    }else{
      partIdCounter.current=nextPlacedParts.length;
    }

    const firstPart=partsRef.current[0]?.mesh;
    if(firstPart&&controlsRef.current){
      controlsRef.current.target.copy(firstPart.position);
      controlsRef.current.update();
    }
  },[applyTextureToMesh,clearStudio]);

  /* ── Save Project ───────────────────────────────────────────────── */
  const handleSaveProject=useCallback(async ()=>{
    const token=localStorage.getItem("token");
    if(!token||token==="dummy_jwt_token"){
      setSaveStatus({type:"error",message:"Please log in to save projects. Guest users cannot save."});
    ...
    try{
      // Serialize parts data
      const partsData=partsRef.current.map(p=>({
        id:p.id,
        type:p.type,
        ...
        positions:Array.from(p.mesh.geometry.attributes.position.array),
        textureSrc:textures[p.id]||null,
      }));

      const payload={
        project_name:projectName.trim(),
        furniture_type:"chair", // Default to chair for now; can extend this later
        parts:partsData,
        textures:textures||{},
        project_metadata:{partCount:partsData.length,hasSeat},
      };

      const response=await fetch("http://localhost:8000/projects-3d/save",{
        method:"POST",
        headers:{
      ...
      setSaveStatus({type:"success",message:`Project "${projectName}" saved successfully!`});
      setProjectName("");
      setShowSaveModal(false);
      setTimeout(()=>setSaveStatus(null),5000);
    }catch(err){
      setSaveStatus({type:"error",message:err.message||"Failed to save project"});
      console.error("Save error:",err);
    }finally{
      setSaving(false);
    }
  },[projectName,textures,hasSeat]);

  const fetchSavedProjects=useCallback(async ()=>{
    const token=localStorage.getItem("token");
    if(!token||token==="dummy_jwt_token"){
      setSaveStatus({type:"error",message:"Please log in to view saved projects. Guest users cannot access them."});
      setTimeout(()=>setSaveStatus(null),5000);
      return;
    }
    ...
    try{
      const response=await fetch("http://localhost:8000/projects-3d/list",{
        headers:{"Authorization":`Bearer ${token}`},
      });
      if(!response.ok){
        const error=await response.json().catch(()=>({detail:"Failed to load projects"}));
        throw new Error(error.detail||"Failed to load projects");
      }
      const projects=await response.json();
      setSavedProjects(projects);
    }catch(err){
      setSaveStatus({type:"error",message:err.message||"Failed to load projects"});
      setSavedProjects([]);
      setTimeout(()=>setSaveStatus(null),5000);
    }finally{
  ...
  const openProjectsDialog=useCallback(()=>{
    const token=localStorage.getItem("token");
    if(!token||token==="dummy_jwt_token"){
      setSaveStatus({type:"error",message:"Please log in to access saved projects. Guest users cannot open them."});
      setTimeout(()=>setSaveStatus(null),5000);
      return;
    }
  ...
  const openSavedProject=useCallback(async(projectId)=>{
    const token=localStorage.getItem("token");
    if(!token||token==="dummy_jwt_token"){
      setSaveStatus({type:"error",message:"Please log in to open saved projects. Guest users cannot access them."});
      setTimeout(()=>setSaveStatus(null),5000);
      return;
    }
    ...
    try{
      const response=await fetch(`http://localhost:8000/projects-3d/${projectId}`,{
        headers:{"Authorization":`Bearer ${token}`},
      });
      if(!response.ok){
        const error=await response.json().catch(()=>({detail:"Failed to open project"}));
        throw new Error(error.detail||"Failed to open project");
      }
      const project=await response.json();
      loadProjectIntoStudio(project);
      setSaveStatus({type:"success",message:`Opened "${project.project_name}"`});
      setTimeout(()=>setSaveStatus(null),5000);
    }catch(err){
      setSaveStatus({type:"error",message:err.message||"Failed to open project"});
      setTimeout(()=>setSaveStatus(null),5000);
    }finally{
  ...
  const openSaveDialog=useCallback(()=>{
    ...
    if(placedParts.length===0){
      setSaveStatus({type:"error",message:"Add some parts before saving."});
    ...
    <div className="ss-root">
      <header className="ss-header">
        ...
        <div className="ss-header-actions">
          ...
          <button className="ss-hbtn" onClick={clearStudio}>Clear All</button>
          <button className="ss-hbtn ss-save-btn" onClick={openProjectsDialog} title="Open a saved 3D project">Open Saved</button>
          <button className="ss-hbtn ss-save-btn" onClick={openSaveDialog} title="Save your 3D design">💾 Save</button>
        </div>
      </header>
        ...
        <aside className="ss-props">
          {selectedPartId&&selPart?<div className="ss-props-content">
            ...
            {toolMode==="sculpt"&&<div className="ss-props-section"><div className="ss-props-stitle">Brush</div>
              ...
              <div className="ss-props-stitle">Material</div>
              {textures[selectedPartId]&&(
                <div className="ss-mat-active">
                  <img src={textures[selectedPartId]} className="ss-mat-thumb" alt="Applied"/>
                  <span className="ss-mat-active-label">Applied</span>
                  <button className="ss-mat-clear" onClick={clearMaterial}>✕</button>
                </div>
              )}
              <MaterialPanel onApply={applyMaterial}/>
            </div>
      ...
      {/* Save Status Message */}
      {saveStatus&&(
        <div className={`ss-toast ss-toast-${saveStatus.type}`}>
          {saveStatus.message}
        </div>
      )}
      ...
      {/* Save Modal */}
      {showSaveModal&&(
        <div className="ss-modal-overlay" onClick={()=>setShowSaveModal(false)}>
          <div className="ss-modal" onClick={e=>e.stopPropagation()}>
            <div className="ss-modal-header">
              <h3>Save Your Design</h3>
              <button className="ss-modal-close" onClick={()=>setShowSaveModal(false)}>✕</button>
            </div>
            <div className="ss-modal-body">
              <label htmlFor="projectNameInput" className="ss-modal-label">Project Name</label>
              <input
                id="projectNameInput"
                type="text"
                className="ss-modal-input"
                placeholder="e.g., My Elegant Chair"
                value={projectName}
                onChange={e=>setProjectName(e.target.value)}
                onKeyDown={e=>{if(e.key==="Enter"&&!saving)handleSaveProject();}}
                disabled={saving}
                autoFocus
              />
              <div className="ss-modal-info">
                <p>Parts: {placedParts.length}</p>
                <p>Type: Chair</p>
              </div>
            </div>
            <div className="ss-modal-footer">
              <button className="ss-modal-btn ss-modal-btn-cancel" onClick={()=>setShowSaveModal(false)} disabled={saving}>
                Cancel
              </button>
              <button className="ss-modal-btn ss-modal-btn-save" onClick={handleSaveProject} disabled={saving||!projectName.trim()}>
      ...
      {showProjectsModal&&(
        <div className="ss-modal-overlay" onClick={()=>setShowProjectsModal(false)}>
          <div className="ss-modal ss-projects-modal" onClick={e=>e.stopPropagation()}>
            <div className="ss-modal-header">
              <h3>Open Saved Project</h3>
              <button className="ss-modal-close" onClick={()=>setShowProjectsModal(false)}>✕</button>
            </div>
            <div className="ss-modal-body ss-projects-body">
              {loadingProjects?(
                <div className="ss-projects-empty">Loading saved projects...</div>
              ):savedProjects.length===0?(
                <div className="ss-projects-empty">No saved projects yet.</div>
              ):(
                <div className="ss-projects-list">
                  {savedProjects.map(project=>(
                    <button key={project.id} className="ss-project-card" onClick={()=>openSavedProject(project.id)} disabled={loadingProjectId===project.id}>
                      <div className="ss-project-card-head">
                        <span className="ss-project-card-name">{project.project_name}</span>
                        <span className="ss-project-card-type">{project.furniture_type}</span>
                      </div>
                      <div className="ss-project-card-meta">
                        <span>{new Date(project.updated_at||project.created_at).toLocaleString()}</span>
                        <span>{loadingProjectId===project.id?"Opening...":"Open"}</span>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="ss-modal-footer">
              <button className="ss-modal-btn ss-modal-btn-cancel" onClick={()=>setShowProjectsModal(false)}>
                Close
              </button>
            </div>
          </div>
...
function MaterialPanel({onApply}){
  const[openCat,setOpenCat]=useState(null);
  return(
    <div className="ss-mat-wrap">
      {Object.entries(MATERIAL_DATA).map(([cat,subCats])=>{
        const isOpen=openCat===cat;
        return(
          <div key={cat} className="ss-mat-cat">
            <button className={`ss-mat-cat-btn ${isOpen?"open":""}`} onClick={()=>setOpenCat(isOpen?null:cat)}>
              <span>{cat[0].toUpperCase()+cat.slice(1)}</span>
              <svg className={`ss-inv-chev ${isOpen?"open":""}`} width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
            {isOpen&&(
              <div className="ss-mat-subcats">
                {Object.entries(subCats).map(([sub,swatches])=>(
                  <div key={sub} className="ss-mat-subcat">
                    <div className="ss-mat-subname">{sub}</div>
                    <div className="ss-mat-swatches">
                      {swatches.map((sw,i)=>(
                        <button key={i} className="ss-mat-swatch" onClick={()=>onApply(sw.src)} title={sw.name}>
